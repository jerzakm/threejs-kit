...
export function applyBoxUV(
	...
	if (boxSize === undefined) {
		...
		const bbox = geom.boundingBox as Box3;
...
export const generateBoxUVs = (geometry: BufferGeometry, uvMapSize?: number) => {
	if (!uvMapSize) {
		geometry.computeBoundingBox();
		const bboxSize = new Vector3();
		geometry.boundingBox?.getSize(bboxSize);
		uvMapSize = Math.max(bboxSize.x, bboxSize.y, bboxSize.z);
	}

	const boxGeometry = new BoxGeometry(uvMapSize, uvMapSize, uvMapSize);
